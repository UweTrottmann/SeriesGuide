...
See the matching classes that document each property:
...
`externalId` matches a TVDB ID in SeriesGuide library. So this does not work for new shows added to 
the library.
...
See the [Movie](/app/src/main/java/com/battlelancer/seriesguide/dataliberation/model/Movie.java) 
class that documents each property.
