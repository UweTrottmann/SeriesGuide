...
interface EpisodeHelper {
    ...
    fun getLastUpdatedInfoForShow(showId: Long): List<SgEpisodeUpdateInfo>
     ...
     */
    @Query(
        """UPDATE episodes SET episode_watched = 1, episode_plays = episode_plays + 1 WHERE series_id=:showTvdbId
            AND (
     ...
     */
    @Query(
        """UPDATE episodes SET episode_watched = 0, episode_plays = 0 WHERE season_id=:seasonTvdbId
     ...
     */
    @Query(
        """UPDATE episodes SET episode_watched = 1, episode_plays = episode_plays + 1 WHERE season_id=:seasonTvdbId
     ...
     */
    @Query(
        """UPDATE episodes SET episode_watched = 2 WHERE season_id=:seasonTvdbId
            AND episode_firstairedms <= :currentTimePlusOneHour
            AND ${Episodes.SELECTION_HAS_RELEASE_DATE}
     ...
     */
    @Query(
        """UPDATE episodes SET episode_watched = 0, episode_plays = 0 WHERE series_id=:showTvdbId
            AND ${Episodes.SELECTION_WATCHED_OR_SKIPPED}
