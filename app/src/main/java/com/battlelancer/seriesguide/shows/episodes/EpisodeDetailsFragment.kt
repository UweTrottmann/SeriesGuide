...
class EpisodeDetailsFragment : Fragment(), EpisodeActionsContract {
    ...
    private fun populateEpisodeData(episode: SgEpisode2, show: SgShow2) {
        ...
        if (overview.isNullOrEmpty()) {
            ...
            overview = TextTools.textNoTranslation(
                requireContext(),
                show.language?.let { LanguageTools.mapLegacyShowCode(it) })
        } else if (hideDetails) {
    ...
    private fun updateSecondaryButtons(episode: SgEpisode2, show: SgShow2) {
        ...
        if (show.tmdbId != null) {
            ...
            bindingBottom.buttonEpisodeTmdb.openUrlOnClickAndCopyOnLongPress(tmdbUrl)
        }
