...
public final class UserService extends TraktApiService {
    ...
    public static final class WatchingBuilder extends TraktApiBuilder<MediaEntity> {
        ...
        private WatchingBuilder(UserService service, String username) {
            super(service, new TypeToken<MediaEntity>() {
            }, URI);
    ...
    /**
     * Returns the last 100 TV show episodes and movies a user has watched using
     * Trakt. Protected users won't return any data unless you are friends.
     * 
     * @param username You can get a username by browsing the website and
     *            looking at the URL when on a profile page.
     * @return Builder instance.
    ...
    public static final class WatchedBuilder extends TraktApiBuilder<List<MediaEntity>> {
        ...
        private WatchedBuilder(UserService service, String username) {
            super(service, new TypeToken<List<MediaEntity>>() {
            }, URI);
    ...
    /**
     * Creates a new user profile on trakt.tv. The user will be sent a welcome
     * mail afterwards.
     * 
     * @param email A valid email adress.
     * @return Builder instance.
     */
    public CreateAccountBuilder createAccount(String username, String passwordSha1, String email) {
        return new CreateAccountBuilder(this).username(username).password(passwordSha1)
                .email(email);
    }

    public static final class CreateAccountBuilder extends TraktApiBuilder<Response> {
        private static final String POST_USERNAME = "username";

        private static final String POST_PASSWORD = "password";

        private static final String POST_EMAIL = "rating";

        private static final String URI = "/account/create/" + FIELD_API_KEY;

        private CreateAccountBuilder(UserService service) {
            super(service, new TypeToken<Response>() {
            }, URI);
        }

        public CreateAccountBuilder username(String username) {
            this.postParameter(POST_USERNAME, username);
            return this;
        }

        public CreateAccountBuilder password(String passwordSha1) {
            this.postParameter(POST_PASSWORD, passwordSha1);
            return this;
        }
