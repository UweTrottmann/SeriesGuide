...
public class NetworkJob {
    ...
    public NetworkJob(EpisodeFlagJob job, boolean shouldSendToHexagon, boolean shouldSendToTrakt) {
        this.job = job;
        this.shouldSendToHexagon = shouldSendToHexagon;
        this.shouldSendToTrakt = shouldSendToTrakt;
    }

    public void tryNetworkJob(Context context) {
        int networkJobResult = doNetworkJob(context);
        handleResult(context, networkJobResult);
    }

    private int doNetworkJob(Context context) {
        // upload to hexagon
    ...
    private void handleResult(Context context, Integer result) {
        ...
        EventBus.getDefault().post(
                new BaseNavDrawerActivity.ServiceCompletedEvent(confirmationText,
                        displaySuccess));
        EventBus.getDefault().post(new EpisodeJobAsyncTask.CompletedEvent(job, isSuccessful));
    }
